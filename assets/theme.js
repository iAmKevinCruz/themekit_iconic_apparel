...
theme.Sections = function Sections() {
  ...
  if (url[4] === 'not-your-daddy') {
    console.log($(".filters-toolbar-wrapper"));
  	$(".filters-toolbar-wrapper").css("display", "none");  
  }
...
slate.Variants = (function() {
  ...
  function Variants(options) {
	...
	this.varitationchange_image(this.currentVariant.option1.toLowerCase());
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _onSelectChange: function() {
      ...
      if (this.enableHistoryState) {
        ...
        this.varitationchange_image(variant.option1.toLowerCase());
      }
    },
     varitationchange_image: function(varitationval){
        if($('.slick-arrow').length){
          $(".product-single__thumbnails").slick('slickUnfilter');
          $('.product-single__thumbnails-item').removeClass('active-filter');
          $('.product-single__thumbnails-item').each(function(){
            var varname = $.trim($(this).data('varname').toLowerCase());
            if(varname == varitationval){
              $(this).addClass('active-filter');
            }

          });
          $(".product-single__thumbnails").slick('slickFilter','.active-filter');
          $(".product-single__thumbnails").slick('slickGoTo',0);
        }else{
          $('.product-single__thumbnails-item').each(function(){
            var varname = $.trim($(this).data('varname').toLowerCase());
            var subin = varname.indexOf(varitationval);
            if(subin > -1){
              $(this).show(); 
              $(this).find('.slick-cloned').show();
            }else{
              $(this).hide();
...
theme.MobileNav = (function() {
  ...
  function toggleMobileNav() {
    if (cache.$mobileNavToggle.hasClass(classes.mobileNavCloseIcon)) {
      $('header').removeClass('nav-drawer-open');
      closeMobileNav();
    } else {
      $('header').addClass('nav-drawer-open');
      openMobileNav();
    }
  }
...
$(document).ready(function() {
  ...
  /* grid-progress-bar-slider start  */
//     var $slider = $('.grid-slider-pro');
//     var $progressBar = $('.grid-slider-collection .progress');
//     var $progressBarLabel = $( '.grid-slider-collection .slider__label' );
...
//     var $slider = $('#Slideshow-'+$section_id);
//     var $progressBar = $('#shopify-section-'+$section_id+' .progress');
//     var $progressBarLabel = $('#shopify-section-'+$section_id+' .slider__label' );
...
//        $progressBarLabel.text( nextSlide );
//     });
//   });
  
  /****slideshow Video button click**********/  
  var videoPlayIcon = ".slideshow__slide .video_play_button";
  var videoPauseIcon = ".slideshow__slide .video_pause_button";
  $(document).on('click',videoPlayIcon,function(){
    var parent = $(this).closest('.slideshow__slide');
    if($('.video-frame-wrap .video-wrap-iframe',parent).length > 0){
      var iframe_clone = $('.video-frame-wrap .video-wrap-iframe',parent);
      iframe_clone.attr('src',iframe_clone.attr('src')+'&autoplay=1');
    }else if($('.video-frame-wrap #video-wrap-mp4',parent).length > 0){
      var video_clone = $('.video-frame-wrap #video-wrap-mp4',parent).trigger('play');
    }
    parent.addClass('video_play');
    $('.video_pause_button',parent).removeClass('hide');
    $('.video_play_button',parent).addClass('hide');
  });
  $(document).on('click',videoPauseIcon,function(){
    var parent = $(this).closest('.slideshow__slide');
    if($('.video-frame-wrap .video-wrap-iframe',parent).length > 0){
      var iframe_clone = $('.video-frame-wrap .video-wrap-iframe',parent);
      iframe_clone.attr('src',iframe_clone.data('src'));
    }else if($('.video-frame-wrap #video-wrap-mp4',parent).length > 0){
      var video_clone = $('.video-frame-wrap #video-wrap-mp4',parent).trigger('pause');
    }
    parent.removeClass('video_play');
    $('.video_play_button',parent).removeClass('hide');
    $('.video_pause_button',parent).addClass('hide');
  });
});
/********slideshow slider end*************/
/******* start sticky header **********/
var headerSticky = function(){
//   var stickyOffset = $('.site-header').offset().top + 25;
...
/******* end sticky header **********/
$(document).ready(function(){
/***contact header slider js start*********/
  $(".announcebar-slider").slick({
    slidesToShow: 1,
    slidesToScroll: 1,
    swipeToSlide: true,
    vertical: true,
  ...
  headerSticky();
/***contact header slider js end*********/
  $(document).on('click', 'a[href="/timer-popup"]',function(e){
    e.preventDefault();
  	$('#timer_popup_section').show();
    $('body').addClass('open_popup');
    $(window).scrollTop(0);
  });
  $(document).on('click', '.timer_popup_close',function(e){
    e.preventDefault();
    $('#timer_popup_section').hide();
